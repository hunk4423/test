...
while {true} do {
	...
	if (!r_player_infected && !_isPZombie) then {
		...
		//Infectionriskstart
		if (dayz_temperatur < 34) then {	//если у тебя 34 град
			//если рядом зараженный
			...
			//Если переохладился
			if (dayz_temperatur < 28) then {	//если у тебя 28 град
				r_player_infected = true;
				player setVariable["USEC_infected",true];
			};
		};
		
		//Если в машине с зараженными
	...
	if (dayz_temperatur < 34) then { //если у тебя меньше 34 град
		_diff = -20;
		
		if (dayz_temperatur < 30) then {
		_diff = _diff - 20;
		};
		
		if (dayz_temperatur < 28) then {
		_diff = _diff - 20;
		};
		
		r_player_blood = r_player_blood + _diff;
	};
	...
	if (r_player_infected) then {
		...
		if (_rnd < 3) then {
			...
			r_player_blood = r_player_blood - 6;
	};
