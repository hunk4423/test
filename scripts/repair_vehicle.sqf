private ["_part","_color","_allFixed","_percent","_string","_handle","_damage","_cmpt","_vehicle","_hitpoints"];
...
{			
	...
	if (_damage > 0) then {
		if((_damage>=0.8)&&((_part=="PartEngine")||(_part=="PartFueltank")))then{_damage=1;};
		_allFixed = false;
		call{
		...
		_string = format["<t %2>Починить %1 (%3%4)</t>",_cmpt,_color,_percent,"%"]; //Repair - Part
